...
const ChatView = () => {
  ...
  const isUserScrollingUp = useRef(false);           // Tracks if user is reading old messages
  const previousMessagesLength = useRef(0);          // To detect new incoming messages
  ...
  // Send delivery confirmation when chat opens
  ...
  useEffect(() => {
    ...
    const lastMessageIsMine = messages[messages.length - 1]?.sender === "manager";
    ...
    if (newMessagesArrived && (isNearBottom || lastMessageIsMine || !isUserScrollingUp.current)) {
      scrollToBottom();
    }
  }, [messages]);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Optional: Re-enable auto-scroll after 8 seconds of inactivity
  ...
  const StatusIcon = ({ status }) => {
    ...
    const style = { marginLeft: '5px', fontSize: '0.9rem', verticalAlign: 'middle' };

    if (s === 'seen') return <IoCheckmarkDone style={{ ...style, color: '#4fc3f7' }} />;
      ...
      if (data.head.code === 200) {
        ...
        setMessages(prev =>
          prev.map(m => m.message_id === tempId
            ? { ...m, message_id: data.body.message_id || tempId, status: "delivered" }
            : m
          )
        );
      } else {
  ...
  const ChatMessage = ({ msg }) => {
    ...
    const bubble = isCustomer ? "chat-bubble customer-bubble" : "chat-bubble manager-bubble";
      ...
      <Row className={`mb-3 ${align}`}>
        <Col xs={10} sm={8} lg={6}>
          <div className={bubble}>
            <div className="sender-name"><strong>{name}</strong></div>
  ...
  return (
    <Container fluid className="chat-view-container d-flex flex-column" style={{ height: "100vh" }}>
      {/* Header */}
      ...
      <div
        ...
        className="chat-messages-area p-3 "
      >
        {loading && messages.length === 0 ? (
          <div className="text-center py-5 text-muted">{t("Loading chat history...")}</div>
        ) : messages.length === 0 ? (
          <div className="text-center py-5 text-muted">{t("No messages found for this customer.")}</div>
        ) : (
          messages.map((msg) => (
            <ChatMessage
      ...
      {/* Input */}
      <form onSubmit={handleSendMessage} className="chat-input-row py-2 mb-3 sticky-bottom">
