...
import { Container, Col, Row, Tabs, Tab, Modal, Form, Button, Card } from "react-bootstrap";
  ...
  const handleSaveBanner = async () => {
    ...
    try {
      ...
      if (responseData.head.code === 200) {
        toast.success(responseData.head.msg || t("Banner saved successfully!"), {
          position: "top-center", autoClose: 2000, hideProgressBar: false, closeOnClick: true, pauseOnHover: true, draggable: true, progress: undefined, theme: "colored",
        });
        handleCloseModal();
        fetchData(); 
      } else {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (dataOne.head.code === 200) {
        setBannerOneData(Array.isArray(dataOne.body.banner_one) ? dataOne.body.banner_one : []);
      } else {
        setBannerOneData([]);
      }
      ...
      if (dataTwo.head.code === 200) {
        setBannerTwoData(Array.isArray(dataTwo.body.banner_two) ? dataTwo.body.banner_two : []);
      } else {
        setBannerTwoData([]);
      }
    ...
    return (
      <Row className="mt-3">
        {data.map((item, index) => {
          ...
          return (
            <Col lg={3} md={4} sm={6} xs={12} key={item.id || index} className="mb-4">
              <Card>
    ...
    <div>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>{t("Upload New Banner Image")} ({activeTab === 'bannerOne' ? t("Banner One") : t("Banner Two")})</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            ...
            <Form.Group className="mb-3">
              <p className="fw-bold mb-1">{t("Base64 Encode Status")}</p>
              <textarea 
                  className="form-control small" 
                  rows="3" 
                  readOnly 
                  value={base64Image ? t("Image encoded successfully (Base64 string ready)") : t("Encode will appear here")}
              />
            </Form.Group>
          </Form>
        </Modal.Body>
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleSaveBanner} disabled={!base64Image || loading}>
      ...
      >
          ...
          <Modal.Body>
              <p className="lead">{t("Are you sure you want to delete this banner?")}</p>
